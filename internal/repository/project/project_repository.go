...
func NewProjectRepository(apiUrl, authToken string) *ProjectRepository {
...
func (ur *ProjectRepository) GetAllProjects(ctx context.Context, path string) ([]byte, error) {
	var op = "GetAllProjects"

	url := fmt.Sprintf("%s/%s", ur.apiUrl, path)

	req, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		slog.Error(fmt.Sprintf("failed to get all projects, err: %s, op: %s", err.Error(), op))
		return nil, err
	}
...
func (ur *ProjectRepository) GetProjectByGID(ctx context.Context, path string) ([]byte, error) {
	var op = "GetProjectByGID"
